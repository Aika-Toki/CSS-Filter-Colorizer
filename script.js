...
function generate() {
  const rgb = hexToRgb(document.querySelector("#targetColorHex").value);
  if (rgb.length !== 3) {
  ...
  setTimeout(() => {
    ...
    document.querySelector("#targetColorPreview").style.backgroundColor =
      color.toString();
    document.querySelector("#resultColorPreview>label").style.color =
      color.toString();
    document
      .querySelector("#resultColorPreview")
      .setAttribute("style", result.filter);
    document.querySelector("#result").textContent = result.filter;
